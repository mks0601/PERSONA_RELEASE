...
os.system('cp ' + osp.join(root_path, 'images', '*') + ' ./input/.')
frame_idx_list = sorted([int(x.split('/')[-1][:-4]) for x in glob(osp.join('./input/*.png'))])
...
os.system('./depth.sh')
os.makedirs(osp.join(root_path, 'depths'), exist_ok=True)
if vis_format == 'image':
    os.system('mv ' + osp.join(output_path, '*') + ' ' + osp.join(root_path, 'depths', '.'))
    for frame_idx in frame_idx_list:
        os.system('cp ' + osp.join(output_mask_path, str(frame_idx) + '.npy') + ' ' + osp.join(root_path, 'depths', str(frame_idx) + '_mask.npy'))
else:
    make_video(osp.join(root_path, 'depths.mp4'), output_path)
    os.system('mv ' + osp.join(output_path, '*.npy') + ' ' + osp.join(root_path, 'depths', '.'))
    for frame_idx in frame_idx_list:
        os.system('cp ' + osp.join(output_mask_path, str(frame_idx) + '.npy') + ' ' + osp.join(root_path, 'depths', str(frame_idx) + '_mask.npy'))
