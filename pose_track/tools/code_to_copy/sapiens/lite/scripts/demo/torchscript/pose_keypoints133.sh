...
INPUT=./scripts/demo/$MODE/input
OUTPUT=./scripts/demo/$MODE/output_pose

#--------------------------MODEL CARD---------------
# MODEL_NAME='sapiens_0.3b'; CHECKPOINT=./scripts/demo/$MODE/checkpoints/sapiens_0.3b_coco_wholebody_best_coco_wholebody_AP_620_$MODE.pt2
# MODEL_NAME='sapiens_0.6b'; CHECKPOINT=./scripts/demo/$MODE/checkpoints/sapiens_0.6b_coco_wholebody_best_coco_wholebody_AP_695_$MODE.pt2
MODEL_NAME='sapiens_1b'; CHECKPOINT=./scripts/demo/$MODE/checkpoints/sapiens_1b_coco_wholebody_best_coco_wholebody_AP_727_$MODE.pt2
# MODEL_NAME='sapiens_2b'; CHECKPOINT=./scripts/demo/$MODE/checkpoints/sapiens_2b_coco_wholebody_best_coco_wholebody_AP_745_$MODE.pt2

DETECTION_CONFIG_FILE='../pose/demo/mmdetection_cfg/rtmdet_m_640-8xb32_coco-person_no_nms.py'
DETECTION_CHECKPOINT=./scripts/demo/$MODE/checkpoints/rtmdet_m_8xb32-100e_coco-obj365-person-235e8209.pth

#---------------------------VISUALIZATION PARAMS--------------------------------------------------
LINE_THICKNESS=3 ## line thickness of the skeleton
RADIUS=2 ## keypoint radius
KPT_THRES=0.3 ## confidence threshold
THICKNESS=1 ## keypoint thickness
...
for ((i=0; i<TOTAL_JOBS; i++)); do
  ...
  CUDA_VISIBLE_DEVICES=${VALID_GPU_IDS[GPU_ID]} python ${RUN_FILE} \
    ...
    --radius ${RADIUS} \
    --kpt-thr ${KPT_THRES} \
    --thickness ${THICKNESS} ## add & to process in background
