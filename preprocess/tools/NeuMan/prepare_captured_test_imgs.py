...
if split == 'captured':
    ...
    with open(osp.join(root_path, 'test_split.txt')) as f:
        frame_idx_list = [int(x[:-5]) for x in f.readlines()]

    with open(osp.join(save_path, split, 'frame_idx_orig.txt'), 'w') as f:
...
for i, frame_idx in enumerate(tqdm(frame_idx_list)):
    ...
    # load image
    img = cv2.imread(osp.join(root_path, 'images', '%05d.png' % frame_idx))
