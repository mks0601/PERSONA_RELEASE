...
def preprocess(video_path, image_path, resolution=576, sample_stride=2):
    ...
    """
    # export the original video_pose
    import cv2
    video_height, video_width = video_pose.shape[2:]
    video_save = cv2.VideoWriter('video_pose.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (video_width, video_height))
    for i in range(len(video_pose)):
        video_save.write(video_pose[i].transpose(1,2,0))
    video_save.release()
    """
    
    # load pre-made video_pose
    import cv2
    video_pose = []
    vidcap = cv2.VideoCapture(video_path)
    success, frame = vidcap.read()
    while success:
        video_pose.append(frame)
        success, frame = vidcap.read()
    video_pose = np.stack(video_pose).transpose(0,3,1,2)
