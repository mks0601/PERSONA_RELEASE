...
from pytorch3d.renderer import OrthographicCameras, PerspectiveCameras, RasterizationSettings, MeshRasterizer, TexturesVertex
import numpy as np
from config import cfg

def rasterize_uv(vertex_uv, face_uv, uvmap_shape):
...
def rasterize_xy(vert, face, cam_param, render_shape):
    ...
    raster_settings = RasterizationSettings(image_size=render_shape, blur_radius=0.0, faces_per_pixel=1, cull_backfaces=True)
    rasterizer = MeshRasterizer(cameras=cameras, raster_settings=raster_settings).cuda()
    outputs = rasterizer(mesh)
    return outputs

def render_vertex_texture(fragments, vert_mask, face):
    # render texture
...
class XY2UV(nn.Module):
    def __init__(self, vertex_uv, face_uv, uvmap_shape):
        ...
        outputs = rasterize_uv(vertex_uv, face_uv, uvmap_shape)
    ...
    def forward(self, img, vert_cam, face, cam_param):
        batch_size, channel_dim, img_height, img_width = img.shape

        # project mesh
        x = vert_cam[:,:,0] / vert_cam[:,:,2] * cam_param['focal'][:,None,0] + cam_param['princpt'][:,None,0]
        y = vert_cam[:,:,1] / vert_cam[:,:,2] * cam_param['focal'][:,None,1] + cam_param['princpt'][:,None,1]
        vert_img = torch.stack((x,y),2)

        # get visible faces from mesh
        outputs = rasterize_xy(vert_cam, face, cam_param, (img_height, img_width))
        ...
        vert_img_0, vert_img_1, vert_img_2, invisible_uv = [], [], [], []
        for i in range(batch_size):
            ...
            # get 2D coordinates
            vert_img_0.append(vert_img[i,vertex_idx_0.view(-1),:].view(self.uvmap_shape[0], self.uvmap_shape[1], 2))
            vert_img_1.append(vert_img[i,vertex_idx_1.view(-1),:].view(self.uvmap_shape[0], self.uvmap_shape[1], 2))
            vert_img_2.append(vert_img[i,vertex_idx_2.view(-1),:].view(self.uvmap_shape[0], self.uvmap_shape[1], 2))
        vert_img_0 = torch.stack(vert_img_0) # batch_size, self.uvmap_shape[0], self.uvmap_shape[1], 2
        vert_img_1 = torch.stack(vert_img_1)
        vert_img_2 = torch.stack(vert_img_2)
        ...
        vert_img = vert_img_0 * self.bary_coords_uv[None,:,:,0,None] + vert_img_1 * self.bary_coords_uv[None,:,:,1,None] + vert_img_2 * self.bary_coords_uv[None,:,:,2,None]
        vert_img = torch.stack((vert_img[:,:,:,0]/(img_width-1)*2-1, vert_img[:,:,:,1]/(img_height-1)*2-1), 3) # [-1,1] normalization
       
        # fill UV map
        uvmap = F.grid_sample(img, vert_img, align_corners=True)
