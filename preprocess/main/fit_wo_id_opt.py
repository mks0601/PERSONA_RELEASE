...
def main():
    args = parse_args()
    cfg.set_args(args.subject_id, split='test')
    
    trainer = Trainer()
    trainer._make_batch_generator()
    trainer._make_model(id_opt=False)
    ...
    for frame_idx in trainer.smplx_params.keys():
        ...
        for key in ['jaw_pose', 'leye_pose', 'reye_pose']:
            smplx_params[frame_idx][key] = nn.Parameter(axis_angle_to_rotation_6d(trainer.flame_params[frame_idx][key].cuda())) 
        smplx_params[frame_idx]['expr'] = nn.Parameter(trainer.flame_params[frame_idx]['expr'] .cuda())
        smplx_params[frame_idx]['trans'] = nn.Parameter(trainer.smplx_params[frame_idx]['trans'].cuda()) 

    # load optimized ID parameters (do not optimize them)
    id_root_path = osp.join('..', 'data', 'subjects', cfg.subject_id, 'captured', 'smplx_optimized')
    with open(osp.join(id_root_path, 'shape_param.json')) as f:
        smplx_shape = torch.FloatTensor(json.load(f)).cuda().view(-1)
    with open(osp.join(id_root_path, 'face_offset.json')) as f:
        face_offset = torch.FloatTensor(json.load(f)).cuda().view(-1,3)[smpl_x.face_vertex_idx,:]
    with open(osp.join(id_root_path, 'joint_offset.json')) as f:
    ...
    for epoch in range(cfg.end_epoch):
        ...
        for itr_data, data in enumerate(trainer.batch_generator):
            ...
            for itr_opt in range(cfg.itr_opt_num):
                ...
                # inputs
                smplx_inputs = {'shape': [smplx_shape for _ in range(batch_size)], 'face_offset': [face_offset for _ in range(batch_size)], 'joint_offset': [joint_offset for _ in range(batch_size)]}
                for frame_idx in data['frame_idx']:
                    for key in ['root_pose', 'body_pose', 'jaw_pose', 'leye_pose', 'reye_pose', 'lhand_pose', 'rhand_pose', 'expr', 'trans']:
