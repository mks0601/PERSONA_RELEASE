...
def main():
    args = parse_args()
    cfg.set_args(args.subject_id, split='captured')
    
    trainer = Trainer()
    trainer._make_batch_generator()
    trainer._make_model(id_opt=True)
    
    # register initial flame parameters
    flame_params = {}
    for frame_idx in trainer.flame_params.keys():
        flame_params[frame_idx] = {}
        for key in ['root_pose', 'jaw_pose', 'leye_pose', 'reye_pose']:
            flame_params[frame_idx][key] = nn.Parameter(axis_angle_to_rotation_6d(trainer.flame_params[frame_idx][key].cuda()))
        flame_params[frame_idx]['neck_pose'] = axis_angle_to_rotation_6d(trainer.flame_params[frame_idx]['neck_pose'].cuda()) # do not optimize neck pose
            ...
            for itr_opt in range(cfg.itr_opt_num):
                ...
                if itr_opt == 0:
                    if epoch == (cfg.end_epoch - 1):
                        ...
                        optimizable_params = [smplx_shape, flame_shape, face_offset, joint_offset] # all shared parameters
                    for frame_idx in data['frame_idx']:
                        ...
                        for key in ['root_pose', 'jaw_pose', 'leye_pose', 'reye_pose', 'expr', 'trans']:
                ...
                loss, out = trainer.model(smplx_inputs, flame_inputs, data, return_output=(itr_opt==cfg.itr_opt_num-1))
